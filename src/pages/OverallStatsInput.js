...
export default function OverallStatsInput({ branchcode, handleChange2, setData }) {
    const navigate = useNavigate();

    async function getOverallStats() {
        const response = await fetch('https://sem-result-server.onrender.com/getoverallstats?&branchcode=' + branchcode.toUpperCase(), {
            mode: 'cors',
            method: 'GET'
        });
